...
export const CelebrityHero = ({ profile }: CelebrityHeroProps) => {
  return (
    ...
    >
      ...
      <div className="mx-auto flex h-60 w-60 items-center justify-center rounded-full border border-amber-200/45 bg-[#2A170C] p-1 shadow-[0_0_0_2px_rgba(245,158,11,0.2),0_0_52px_rgba(249,115,22,0.34)]">
      ...
      <div className="mt-4 space-y-2 text-center">
        <p className="text-[11px] uppercase tracking-[0.28em] text-[#F3D9A2]">
          Central oficial
        </p>
        <h1 className="font-['Inter_Tight',sans-serif] text-3xl font-semibold tracking-tight text-[#FFF4DA]">
          {profile.displayName}
        </h1>
        <p className="text-xs uppercase tracking-[0.22em] text-[#F6D8A4]/85">
          {profile.handle}
        </p>
        <p className="text-sm text-[#F6D8A4]">{profile.subtitle}</p>
        <p className="text-sm leading-relaxed text-[#F8E7C2]/92">
          {profile.description}
        </p>
      </div>
        ...
        {profile.badges.map((badge) => (
          ...
          >
            {badge === "Perfil oficial" ? (
              <BadgeCheck className="h-3.5 w-3.5 text-[#F2B233]" />
            ) : null}
            {badge}
          </span>
        ))}
